defmodule TravelpalWeb.FlightController do
  use TravelpalWeb, :controller

  alias Travelpal.ExternalAPI
  alias Travelpal.ExternalAPI.Flight

  action_fallback(TravelpalWeb.FallbackController)

  def index(conn, _params) do
    flights = Flight.list_flights()
    render(conn, "index.json", flights: flights)
  end

  # Returns all of the flights in the database
  def retrieve_all_flights(conn, _params) do
    flights = ExternalAPI.list_flights()
    |> Enum.map(fn(x) -> Map.take(x,
      [:origin, :dest, :date_from, :date_to, :price, :airlines, :duration]) end)

    render(conn, "show.json", flight: flights)
  end

  # Returns all flights that match the given arguments
  def search(conn, %{"origin" => origin, "dest" => dest,
    "date_from" => date_from, "date_to" => date_to}) do
    decoded_origin = URI.decode(origin)
    decoded_dest = URI.decode(dest)
    decoded_date_from = URI.decode(date_from)
    decoded_date_to = URI.decode(date_to)
    existing_data =
      ExternalAPI.get_flights_by_params(decoded_origin, decoded_dest, decoded_date_from, decoded_date_to)
    flights = if (!Enum.empty?(existing_data)),
      do: existing_data,
      else: request_flights(decoded_origin, decoded_dest, decoded_date_from, decoded_date_to)

    if (Enum.empty?(existing_data)) do
      flights = Enum.map(flights, fn(x) -> 
        {:ok, record} = ExternalAPI.create_flight(x)
        record
      end)
    end

    flights = Enum.map(flights, fn(x) -> 
      Map.take(x, [:id, :origin, :dest, :date_from, :date_to, :price, :airlines, :duration, :arrival_time, :departure_time]) end)

    render(conn, "show.json", flight: flights)
  end

  defp request_flights(origin, dest, date_from, date_to) do
    flight_url = "https://api.skypicker.com/flights"
    split_date_from = String.split(date_from, "/")
    converted_date_from = Enum.at(split_date_from, 1) <> "/" <>
                          Enum.at(split_date_from, 0) <> "/" <>
                          Enum.at(split_date_from, 2)
    split_date_to = String.split(date_to, "/")
    converted_date_to = Enum.at(split_date_to, 1) <> "/" <>
                        Enum.at(split_date_to, 0) <> "/" <>
                        Enum.at(split_date_to, 2)
    # gets top 5 flights
    uri = URI.encode(flight_url
      <> "?flyFrom=#{origin}&to=#{dest}&dateFrom=#{converted_date_from}&dateTo=#{converted_date_from}"
      <> "&returnFrom=#{converted_date_to}&returnTo=#{converted_date_to}&partner=picky&partner_market=us&curr=USD&limit=1")

    # Comment out HTTP request for dev purposes and use dummy data instead
    res = HTTPoison.get!(uri)
    data = Poison.decode!(res.body)

    # dummy_data()
    data["data"]
    |> Enum.map(fn(x) -> format_flight(x, date_from, date_to) end)
  end

  defp format_flight(flight, date_from, date_to) do
    # parses the dates and converts them to Elixir Date objects
    split_date_from = String.split(date_from, "/")
    {:ok, formatted_date_from} = Date.new(
      Enum.at(split_date_from, 2) |> String.to_integer(),
      Enum.at(split_date_from, 0) |> String.to_integer(),
      Enum.at(split_date_from, 1) |> String.to_integer()
    )
    split_date_to = String.split(date_to, "/")
    {:ok, formatted_date_to} = Date.new(
      Enum.at(split_date_to, 2) |> String.to_integer(),
      Enum.at(split_date_to, 0) |> String.to_integer(),
      Enum.at(split_date_to, 1) |> String.to_integer()
    )

    %{
      origin: flight["mapIdfrom"]
        |> String.split("-")
        |> Enum.map(fn(x)->String.capitalize(x) end)
        |> Enum.join(" "),
      dest: flight["mapIdto"]
        |> String.split("-")
        |> Enum.map(fn(x)->String.capitalize(x) end)
        |> Enum.join(" "),
      date_from: formatted_date_from,
      date_to: formatted_date_to,
      price: flight["price"],
      airlines: flight["airlines"],
      duration: flight["duration"],
      arrival_time: flight["aTime"] |> Integer.to_string(),
      departure_time: flight["dTime"] |> Integer.to_string()
    }
  end

  defp dummy_data() do
    [
      %{
        "airlines" => ["AS", "UA"],
        "deep_link" =>
          "https://www.kiwi.com/deep?from=BOS&to=LAX&departure=05-08-2018&return=05-12-2018&flightsId=3571213871549614_0%7C3927455603931439_0&price=320&passengers=1&affilid=picky&lang=en&currency=EUR&booking_token=",
        "duration" => %{"departure" => 22800, "return" => 19200, "total" => 42000},
        "mapIdfrom" => "boston",
        "mapIdto" => "los-angeles",
        "price" => 320.0,
        "dTime" => 1525157400,
        "aTime" => 1525305300
      },
      %{
        "airlines" => ["UA"],
        "deep_link" =>
          "https://www.kiwi.com/deep?from=BOS&to=LAX&departure=05-08-2018&return=05-12-2018&flightsId=3571213871549614_0%7C3927455603931456_0&price=325&passengers=1&affilid=picky&lang=en&currency=EUR&booking_token=",
        "duration" => %{"departure" => 22800, "return" => 19380, "total" => 42180},
        "mapIdfrom" => "boston",
        "mapIdto" => "los-angeles",
        "price" => 325.0,
        "dTime" => 1525157400,
        "aTime" => 1525305300
      },
      %{
        "airlines" => ["B6", "AS"],
        "deep_link" =>
          "https://www.kiwi.com/deep?from=BOS&to=LAX&departure=05-08-2018&return=05-12-2018&flightsId=4009918922031857_0%7C3927455603931439_0&price=332&passengers=1&affilid=picky&lang=en&currency=EUR&booking_token=",
        "duration" => %{"departure" => 22920, "return" => 19200, "total" => 42120},
        "mapIdfrom" => "boston",
        "mapIdto" => "los-angeles",
        "price" => 332.0,
        "dTime" => 1525157400,
        "aTime" => 1525305300
      },
      %{
        "airlines" => ["B6", "UA"],
        "deep_link" =>
          "https://www.kiwi.com/deep?from=BOS&to=LAX&departure=05-08-2018&return=05-12-2018&flightsId=4009918922031857_0%7C3927455603931456_0&price=337&passengers=1&affilid=picky&lang=en&currency=EUR&booking_token=",
        "duration" => %{"departure" => 22920, "return" => 19380, "total" => 42300},
        "mapIdfrom" => "boston",
        "mapIdto" => "los-angeles",
        "price" => 337.0,
        "dTime" => 1525157400,
        "aTime" => 1525305300
      },
      %{
        "airlines" => ["UA"],
        "deep_link" =>
          "https://www.kiwi.com/deep?from=BOS&to=LAX&departure=05-08-2018&return=05-12-2018&flightsId=3571213871549614_0%7C3927455603931455_0&price=339&passengers=1&affilid=picky&lang=en&currency=EUR&booking_token=",
        "duration" => %{"departure" => 22800, "return" => 19380, "total" => 42180},
        "mapIdfrom" => "boston",
        "mapIdto" => "los-angeles",
        "price" => 339.0,
        "dTime" => 1525157400,
        "aTime" => 1525305300
      }
    ]
  end
end
